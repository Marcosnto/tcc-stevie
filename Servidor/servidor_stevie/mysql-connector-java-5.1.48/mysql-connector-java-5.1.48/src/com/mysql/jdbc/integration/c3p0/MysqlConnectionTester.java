...
/**
 * ConnectionTester for C3P0 com.example.stevie.connection pool that uses the more efficient COM_PING method of testing com.example.stevie.connection 'liveness' for MySQL, and 'sorts' exceptions
...
public final class MysqlConnectionTester implements QueryConnectionTester {
    ...
    public MysqlConnectionTester() {
        try {
            ...
            // punt, we have no way to recover, other than we now use 'SELECT 1' for handling the com.example.stevie.connection testing.
        }
    }
        ...
        try {
            if (this.pingMethod != null) {
                if (con instanceof com.mysql.jdbc.Connection) {
                    // We've been passed an instance of a MySQL com.example.stevie.connection -- no need for reflection
                    ((com.mysql.jdbc.Connection) con).ping();
                } else {
                    // Assume the com.example.stevie.connection is a C3P0 proxy
