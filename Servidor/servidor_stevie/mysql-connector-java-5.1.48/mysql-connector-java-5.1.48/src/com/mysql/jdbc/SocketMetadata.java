...
public interface SocketMetadata {
    ...
    /*
     * Provides a standard way of determining whether a socket com.example.stevie.connection is local.
     * 
        ...
        public static boolean isLocallyConnected(com.mysql.jdbc.ConnectionImpl conn) throws SQLException {
            ...
            if (System.getProperty(IS_LOCAL_HOSTNAME_REPLACEMENT_PROPERTY_NAME) != null) {
                ...
                try {
                    ...
                    if (processHost == null) {
                        // http://bugs.mysql.com/bug.php?id=44167 - com.example.stevie.connection ids on the wire wrap at 4 bytes even though they're 64-bit numbers
                        conn.getLog()
                        ...
                        if (rs.next()) {
                            ...
                            conn.getLog().logError(
                                    "No rows returned for statement \"SELECT CONNECTION_ID()\", local com.example.stevie.connection check will most likely be incorrect");
                        }
                    }
                } finally {
