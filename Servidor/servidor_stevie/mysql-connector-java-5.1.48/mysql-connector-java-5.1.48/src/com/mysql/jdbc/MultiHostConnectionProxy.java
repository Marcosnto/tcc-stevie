...
public abstract class MultiHostConnectionProxy implements InvocationHandler {
    ...
    /**
     * Proxy class to intercept and deal with errors that may occur in any object bound to the current com.example.stevie.connection.
     */
    ...
    /**
     * Initializes a com.example.stevie.connection wrapper for this MultiHostConnectionProxy instance.
     * 
     * @param props
    ...
    /**
     * Constructs a MultiHostConnectionProxy instance for the given list of hosts and com.example.stevie.connection properties.
     * 
     * @param hosts
    ...
    /**
     * Initializes the hosts lists and makes a "clean" local copy of the given com.example.stevie.connection properties so that it can be later used to create standard
     * connections.
     * 
     * @param hosts
     *            The list of hosts for this multi-host com.example.stevie.connection.
     * @param props
    ...
    /**
     * Get this com.example.stevie.connection's proxy.
     * A multi-host com.example.stevie.connection may not be at top level in the multi-host connections chain. In such case the first com.example.stevie.connection in the chain is available as a
     * proxy.
     * 
     * @return
     *         Returns this com.example.stevie.connection's proxy if there is one or itself if this is the first one.
     */
    ...
    /**
     * Sets this com.example.stevie.connection's proxy. This proxy should be the first com.example.stevie.connection in the multi-host connections chain.
     * After setting the com.example.stevie.connection proxy locally, propagates it through the dependant connections.
     * 
     * @param proxyConn
    ...
    /**
     * Propagates the com.example.stevie.connection proxy down through the multi-host connections chain.
     * This method is intended to be overridden in subclasses that manage more than one active com.example.stevie.connection at same time.
     * 
     * @param proxyConn
     *            The top level com.example.stevie.connection in the multi-host connections chain.
     */
    ...
    /**
     * If the given return type is or implements a JDBC interface, proxies the given object so that we can catch SQL errors and fire a com.example.stevie.connection switch.
     * 
     * @param returnType
    ...
    /**
     * Checks if the given throwable should trigger a com.example.stevie.connection switch.
     * 
     * @param t
    ...
    /**
     * Checks if current com.example.stevie.connection is to a master host.
     */
    abstract boolean isMasterConnection();

    /**
     * Invalidates the current com.example.stevie.connection.
     */
    ...
    /**
     * Invalidates the specified com.example.stevie.connection by closing it.
     * 
     * @param conn
     *            The com.example.stevie.connection instance to invalidate.
     */
    synchronized void invalidateConnection(MySQLConnection conn) throws SQLException {
        try {
            ...
            // swallow this exception, current com.example.stevie.connection should be useless anyway.
        }
    }

    /**
     * Picks the "best" com.example.stevie.connection to use from now on. Each subclass needs to implement its com.example.stevie.connection switch strategy on it.
     */
    abstract void pickNewConnection() throws SQLException;

    /**
     * Creates a new physical com.example.stevie.connection for the given host:port.
     * 
     * @param hostPortSpec
    ...
    synchronized ConnectionImpl createConnectionForHost(String hostPortSpec) throws SQLException {
        ...
        if (hostName == null) {
            throw new SQLException("Could not find a hostname to start a com.example.stevie.connection to");
        }
        if (portNumber == null) {
     ...
     * 
     * @param source
     *            The com.example.stevie.connection where to get state from.
     * @param target
     *            The com.example.stevie.connection where to set state.
     */
     ...
     * Subclasses have to override this to complete the method invocation process, deal with exceptions and decide when to switch com.example.stevie.connection.
