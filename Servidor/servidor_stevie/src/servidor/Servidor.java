...
//                        break;
    ...
    public void navegacao(No origem, String destino){
        ...
        while (caminhoAtual.getProximo() != null) {
            ...
            if (tagAtual.equals(caminhoAtual.getNode().getTag()) && enviado == false) {
            ...
            if (tagAtual.equals(caminhoAtual.getProximo().getNode().getTag())) {
                if(caminhoAtual.getProximo().getNode() == caminhoAtual.getNode().getNorte()){
                    new SocketEnviar("direcao:Continue para o norte", ip, 7500).send();
                }else if(caminhoAtual.getProximo().getNode() == caminhoAtual.getNode().getSul()){
                    new SocketEnviar("direcao:Continue para o Sul", ip, 7500).send();
                }else if(caminhoAtual.getProximo().getNode() == caminhoAtual.getNode().getLeste()){
                    new SocketEnviar("direcao:Continue para o Leste", ip, 7500).send();
                }else if(caminhoAtual.getProximo().getNode() == caminhoAtual.getNode().getOeste()){
                    new SocketEnviar("direcao:Continue para o Oeste", ip, 7500).send();
                }
                caminhoAtual = caminhoAtual.getProximo();
            }
            if(!tagAtual.equals(caminhoAtual.getProximo().getNode().getTag()) && !tagAtual.equals(caminhoAtual.getNode().getTag())){
                String localizacaoAtual = lerArq(); //Pega a localização atual, no caso a ultima tag lida
                if (origem != null){
                    origem = bfs.resultado(bfs.buscaBFS(bfs.buscarTag(localizacaoAtual), bfs.buscarTag(destino)));
                    enviado = false;
                }
            }
            try {
